...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if (PG_DIRTY & victim->frame) {
			// Swap out page in advanced
			int offset = swap_pageout(frame, victim->swap_off);
			if (offset != INVALID_SWAP) { victim->swap_off = offset; }
			else { exit(1); }
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// Check if p is valid or not, on swap or not, and handle appropriately
 	if (!(p->frame & PG_VALID)) {
		...
		int pg_onswap = (PG_ONSWAP & p->frame);
		if (pg_onswap && (swap_pagein(f, p->swap_off))) { exit(1); }
		else if (!pg_onswap) { init_frame(f, vaddr); }
		p->frame = f<<PAGE_SHIFT;
		if (!pg_onswap) { p->frame |= PG_DIRTY; }
		miss_count++;
	} else { hit_count++; }

	// Make sure that p is marked valid and referenced. Also mark it
	// dirty if the access type indicates that the page will be written to.
	if (type == 'S'||type == 'M') { p->frame |= PG_DIRTY; }
	p->frame |= (PG_REF|PG_VALID);
	ref_count++;
