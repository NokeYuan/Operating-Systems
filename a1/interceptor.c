 ...
 */
asmlinkage long (*orig_exit_group)(int);
 ...
 *
 */
asmlinkage long my_exit_group(int status) {
	/* Synchronization */
	spin_lock(&my_table_lock);

	/* Remove a pid for all the intercepted system calls */
	del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
	// reg.ax stores system call number
	mytable entry = table[reg.ax];

	// all pids being monitored
	if ((entry.intercepted == 1) && (entry.monitored == 2)) {
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		return (*entry.f)(reg);
	}
	// current pid being monitored
	else if ((entry.intercepted == 1) && (entry.monitored == 1) && (check_pid_monitored(reg.ax, current->pid))) {
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		return (*entry.f)(reg);
	}
	else {
...
asmlinkage long my_syscall_intercept(int syscall) {
	mytable entry = table[syscall];

	// Check for syscall validation
	if ((syscall < 0) || (syscall > NR_syscalls) || (syscall == MY_CUSTOM_SYSCALL)) {
		return -EINVAL;
	}
	// Check for right permission: uid=0 is root
	else if (current_uid() != 0) {
		return -EPERM;
	}
	// Check if already intercepted, if intercepted return -EBUSY
	else if (entry.intercepted == 1) {
		return -EBUSY;
	}
		...
		/* Save the original syscall to table entry*/
		table[syscall].intercepted = 1;
		table[syscall].f = sys_call_table[syscall];

		/* must make it RW before replacing a syscall */
		set_addr_rw((unsigned long)sys_call_table);
		sys_call_table[syscall] = &interceptor; // replace
...
asmlinkage long my_syscall_deintercept(int syscall) {
	...
	// Check for right permission: root checking
	else if (current_uid() != 0) {
		return -EPERM;
	}
	// Check if not have been intercepted, if not return -EBUSY
	else if (entry.intercepted == 0) {
		return -EBUSY;
	}
		...
		/* must make it RW before replacing a syscall */
		set_addr_rw((unsigned long)sys_call_table);
		sys_call_table[syscall] = entry.f; // restore
		set_addr_ro((unsigned long)sys_call_table);
...
asmlinkage long my_syscall_monitor(int syscall, int pid) {
	mytable entry = table[syscall];

	if ((pid < 0) || (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL && pid != 0) || (entry.intercepted == 0)) {
		return -EINVAL;
	}
	// not root
	...
	if (check_pid_monitored(syscall, pid) && entry.monitored == 1) {
		return -EBUSY;
	}
	else {
		spin_lock(&my_table_lock);

		// pid != 0, not monitor for all
		if (pid != 0) {
			// already monitoring all pid
			if (entry.monitored == 2) {
				spin_unlock(&my_table_lock);
				return -EBUSY;
			}
...
asmlinkage long my_syscall_demonitor(int syscall, int pid) {
	mytable entry = table[syscall];
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		...
		return my_syscall_monitor(syscall, pid);
	}	
	else if (cmd == REQUEST_STOP_MONITORING) {
		return my_syscall_demonitor(syscall, pid);
	}	
	else {
		return -EINVAL;
	}
...
static int init_function(void) {
	...
	for(syscall = 1; syscall < NR_syscalls; syscall++){
